...
# def meta_train_format_dataset(data_path):
#     plist, formatted = pqa_format_squad(data_path)
#     ptmatrix = topics_pqa(plist, 100)
#     topicalized, topic_counts = topicalize_pqa(formatted, plist, ptmatrix)
#     split_topicalized, split_topic_counts = support_query_split(topicalized)
...
class MetaTrain:
    def __init__(self, ):
        ...
        self.path = 'save/meta_train_models/checkpoint'
...
#             os.makedirs(self.path)
    ...
    def meta_train(self, model_theta, task_batch, val_dict, tokenizer, eval = False):
        device = self.device
        alpha = .02 # decide on this later
        beta = .1 # decide on this later
        all_gradient = []
        num_task = len(task_batch)
        num_iteration = 10 # TODO decide for both training and eval
        best_scores = {'F1': -1.0, 'EM': -1.0}

        optim_1 = AdamW(model_theta.parameters(), lr=beta)
        model_theta.train()
        ...
        for t, T in enumerate(task_batch):
            ...
            model_phi.to(device)
            optim_2 = AdamW(model_phi.parameters(), lr = alpha)
            model_phi.train()
    ...
    def train(self, model_theta, train_tasks, val_dict, tokenizer):
        ...
        for epoch_num in range(2): 
            task_distribution = get_task_distribution(train_tasks, 16)
            ...
            for i , sampled_train_tasks in enumerate(task_distribution):
                ...
                # if i % 10 == 0:
                #     print("Training step:", i, "with current best_scores: ", best_scores)

                # if (global_step % 10) == 0:
                #     best_scores = self.meta_train(model_theta, task, val_dict, tokenizer, eval = False)

    #                 print("\n----------------- Testing current model -----------------\n")
#                 sampled_test_tasks = sample_tasks(test_tasks, args.batch_size)

#                 for _, task in enumerate(sampled_test_tasks):
#                     best_scores = self.meta_train(model_theta, task, tokenizer, eval = True)
...
#     util.set_seed(args.seed)
    ...
    train_path = "datasets/indomain_train/squad"
    val_path = "datasets/indomain_val/squad"
    
    train_tasks = meta_train_format_dataset(train_path)
    val_dict = read_squad(train_path)
    
    trainer = MetaTrain()
    trainer.train(model, train_tasks, val_dict, tokenizer)
